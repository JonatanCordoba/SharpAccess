    ...
    public async Task SaveOAuthStateAsync(OAuthStateRecord state, CancellationToken cancellationToken = default)
    {
        ...
        await ExecuteAsync(
            ...
            INSERT INTO auth_oauth_states(
                id,provider,state_hash,protected_code_verifier,return_url,created_utc,expires_utc,consumed_utc)
            VALUES(@id,@provider,@stateHash,@protected,@returnUrl,@created,@expires,NULL);
            """,
            cancellationToken,
            ("@id", state.Id),
            ...
            ("@created", ToUtc(state.CreatedUtc)),
            ("@expires", ToUtc(state.ExpiresUtc))).ConfigureAwait(false);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            int consumed = await ExecuteAsync(
                ...
                cancellationToken,
                ("@now", ToUtc(now)),
                ("@id", state.Id)).ConfigureAwait(false);
            if (consumed != 1)
            {
                await transaction.RollbackAsync(cancellationToken).ConfigureAwait(false);
                return null;
